 ...
 * каждый раз, когда сумма покупок по продукту за минуту станет больше, чем MAX_SUM_PER_MINUTE
 */
public class SumAlertsApp {
    public static final String PURCHASE_WITH_PRODUCT_TOPIC_NAME = "purchase_with_joined_product-dsl";
    public static final String RESULT_TOPIC = "sum_alerts-dsl";
    private static final long MAX_SUM_PER_MINUTE = 3000L;
    ...
    public static Topology buildTopology(SchemaRegistryClient client, Map<String, String> serDeConfig) {
        ...
        var purchasesWithProductStream = builder.stream(
                PURCHASE_WITH_PRODUCT_TOPIC_NAME, // указываем имя топика
                Consumed.with(new Serdes.StringSerde(), avroSerde) // указываем тип ключа и тип значения в топике
        );

        Duration oneMinute = Duration.ofMinutes(1);
        purchasesWithProductStream.groupBy((key, val) -> val.get("product_id").toString(), Grouped.with(new Serdes.StringSerde(), avroSerde))
                .windowedBy(
                ...
                .aggregate(
                        () -> 0L,
                        (key, val, agg) -> agg += ((Double) val.get("purchase_sum")).longValue(),
